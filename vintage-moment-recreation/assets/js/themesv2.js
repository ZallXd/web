...
(function () { // webpackBootstrap
    ...
    var __webpack_modules__ = ({
            ...
            (function (__unused_webpack_module, __webpack_exports__, __webpack_require__) {
                ...
                var initCoreInvitation = function initCoreInvitation() {
                    ...
                    if (guestNameSlot) {
                        ...
                        var guestName = urlParams.get('to') || urlParams.get('tamu') || satuMomen.dataset.guest;

                        if (guestName) {
                            guestNameSlot.textContent = guestName;
                        }
                    }
                ...
                __webpack_exports__["default"] = (initCoreInvitation);
        ...
        "./resources/js/invitation-lightbox.js":
            ...
            (function (__unused_webpack_module, __webpack_exports__, __webpack_require__) {
                ...
                /* harmony default export */
                __webpack_exports__["default"] = (initLightBox);
        ...
        "./resources/js/invitation-music.js":
            ...
            (function (__unused_webpack_module, __webpack_exports__, __webpack_require__) {
                ...
                var initMusic = function initMusic() {
                            ...
                            if (musicPlaying) {
                                ...
                                if (playPromise !== undefined) {
                                    playPromise.then(function () {
                                        ...
                                        // Fallback ke muted autoplay jika gagal
                                        if (music && !music.muted) {
                                            music.muted = true;
                                            music.play().then(function () {
                                                console.log("Fallback to muted autoplay success");
                                            }).catch(function (e) {
                                                console.log("Fallback muted autoplay failed:", e);
                                            });
                                        }
                                    });
                                }
                    ...
                    var _markUserInteraction = function markUserInteraction() {
                        ...
                        if (music && !userPausedMusic) {
                            ...
                            if (music.paused) {
                                music.play().then(() => {
                                    console.log("Music unmuted and played on interaction");
                                }).catch((e) => {
                ...
                /* harmony default export */
                __webpack_exports__["default"] = (initMusic);
    ...
    ! function () {
        ...
        __webpack_require__.r(__webpack_exports__);
        /* harmony import */
        var _invitation_core_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__( /*! ./invitation-core.js */ "./resources/js/invitation-core.js");
        /* harmony import */
        var _invitation_music_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__( /*! ./invitation-music.js */ "./resources/js/invitation-music.js");
        /* harmony import */
        var _invitation_lightbox_js__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__( /*! ./invitation-lightbox.js */ "./resources/js/invitation-lightbox.js");
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            if (loader && satuMomen) {
                ...
                // Fallback: Jika load event tidak terjadi dalam 3 detik (dipercepat), sembunyikan loader
        ...
        var rsvpModal = document.getElementById("rsvpModal"); // Define rsvpModal
        ...
        var initRSVP = function initRSVP() {
            ...
            // Placeholder for Google Apps Script URL - User needs to replace this
            var scriptURL = 'https://script.google.com/macros/s/AKfycbx45zGop5xMXx-MRd9MBKVRD1HruJ03VGiSPqZEz1WDMvEXiCI5TqurnEYHocTcK00mIw/exec';

            // Function to fetch and render messages
            var fetchMessages = function () {
                if (!scriptURL.includes('YOUR_GOOGLE_APPS_SCRIPT_URL_HERE')) {
                    if (guestbookList) guestbookList.innerHTML = '<div class="text-center p-3 animate__animated animate__fadeIn">Memuat ucapan...</div>';

                    // Add cache busting
                    var fetchURL = scriptURL + '?v=' + new Date().getTime();
                    fetch(fetchURL)
                        .then(response => response.json())
                        .then(data => {
                            if (guestbookList) {
                                guestbookList.innerHTML = ''; // Clear loading
                                if (Array.isArray(data) && data.length > 0) {
                                    data.forEach(msg => {
                                        // Handle cases where msg might be just values or object
                                        var nama = msg.nama || 'Tamu';
                                        var ucapan = msg.ucapan || '-';
                                        var kehadiran = msg.kehadiran || '';

                                        var item = document.createElement('div');
                                        item.className = 'comment-item mb-2 animate__animated animate__fadeIn';
                                        item.innerHTML = `
                                        <div class="d-flex justify-content-between align-items-center mb-1">
                                            <span class="font-weight-bold" style="font-size: 14px;">${nama}</span>
                                            <span class="badge badge-light" style="font-size: 10px;">${kehadiran}</span>
                                        </div>
                                        <p class="mb-0" style="font-size: 13px;">${ucapan}</p>
                                    `;
                                        guestbookList.appendChild(item);
                                    });
                                } else {
                                    guestbookList.innerHTML = '<div class="text-center p-3 text-muted">Belum ada ucapan. Jadilah yang pertama!</div>';
                                }
                            }
                        })
                        .catch(error => {
                            console.error('Error fetching messages:', error);
                            if (guestbookList) guestbookList.innerHTML = '<div class="text-center p-3 text-danger">Gagal memuat ucapan.</div>';
                        });
                } else if (guestbookList) {
                    guestbookList.innerHTML = '<div class="text-center p-3 text-muted">Setup URL Script Google diperlukan untuk melihat ucapan.</div>';
                }
            };

            // Set name from URL parameter if it exists
            if (guestNameInput && toParam) {
                guestNameInput.value = toParam;
            }
            ...
            if (rsvpForm) {
                rsvpForm.addEventListener("submit", function (e) {
                    ...
                    var attendance = document.getElementById("guest-attendance").value;
                    ...
                    fetch(scriptURL, { method: 'POST', body: formData })
                        .then(response => {
                            ...
                            rsvpForm.reset();
                            // Reset name back to URL param after reset
                            if (toParam) guestNameInput.value = toParam;
                            // Refresh message list
                            fetchMessages();
                        })
                        .catch(error => {
                            console.error('Error!', error.message);
                            // Fallback alert for now since we don't have a real backend yet
                            alert("Simulasi Berhasil: Data akan dikirim ke Google Sheets (Script URL perlu diset).");
                            closeModal(rsvpModal);
                        })
                        .finally(() => {
        ...
        var checkRSVPForm = setInterval(function () {
            ...
            var guestbookList = document.getElementById("guestbook-list"); // Also move guestbook list

            if (rsvpForm && rsvpPlaceholder) {
                rsvpPlaceholder.innerHTML = "";
                rsvpPlaceholder.appendChild(rsvpForm);
                if (guestbookList) rsvpPlaceholder.appendChild(guestbookList);
